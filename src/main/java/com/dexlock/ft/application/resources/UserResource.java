...
//    }
  ...
  @RolesAllowed("Customer")
    ...
    @POST
    @Path("/agent-list/add-agent")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public Response addAgent(User agentData){
        Document insertAgent= daoUser.addAgent(agentData);
        return Response.ok().entity(insertAgent).build();

    }
    @POST
    @Path("/agent-set-free")
    public Response setAgentFree(AccessToken freeAgentData){
        daoUser.setAgentFree(freeAgentData);
