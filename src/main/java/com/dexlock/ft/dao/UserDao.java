...
public class UserDao {
    ...
    private static void sendMail(String userEmail, int num) {
        ...
        final String password = "7857648754756";
    ...
    public Document addAgent(User agentData) {
        Document newAgent = Document.parse(gson.toJson(agentData));
        collectionUser.insertOne(newAgent);
        return (newAgent);
    }

    public void setAgentFree(AccessToken freeAgentData) {

        collectionAccessToken.updateOne(eq("userEmail", freeAgentData.getToken()), Updates.set("status", "free"));
