...
import '../../LoggedInAdmins/ManageRequests/AssignedCardDetails.css';
...
export default class ActiveOrderCardDetails extends React.Component {
  constructor(props) {
    ...
    downloadImage = async () => {
      var taskImage = {
        fileAttachment: this.props.activeTasks.fileAttachment
      }
      await 
      axios({
    ...
    return (
      <SlideToggle
        ...
        render={({ onToggle, setCollapsibleElement, progress }) => (
          <div className="toggle-collapsible1">
            <div class="single-request-card1">
              <button type="button" className="request-card-task-number-button1" onClick={onToggle}>{this.props.activeTasks.customerId}</button>
            </div>
